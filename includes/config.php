<?php
if (!defined('CRON_MODE') && PHP_SAPI !== 'cli' && session_status() === PHP_SESSION_NONE) session_start();
...
<div class="card-header text-white text-center py-3" style="background:#1E3A8A">
...
<div class="card-body text-center py-4">
    ...
    <a href="../modules/index.php" class="btn" style="background:#1E3A8A;color:#fff">
...
function getDB() {
    ...
    if ($conn === null) {
        ...
        $run_core_bootstrap = PHP_SAPI === 'cli'
            || session_status() !== PHP_SESSION_ACTIVE
            || empty($_SESSION['dms_core_bootstrap_v2']);

        if ($run_core_bootstrap) {
        ...
        $cc_res = $conn->query("SELECT COUNT(*) AS c FROM companies");
        $cc = ($cc_res && $cc_res !== false) ? $cc_res->fetch_assoc() : null;
        if ($cc && (int)$cc['c'] === 0) {
            $cs_res = $conn->query("SELECT * FROM company_settings LIMIT 1");
            $cs = ($cs_res && $cs_res !== false) ? $cs_res->fetch_assoc() : null;
            if ($cs) {
        ...
        if (session_status() === PHP_SESSION_ACTIVE) $_SESSION['dms_core_bootstrap_v2'] = 1;
        }
    }
    return $conn;
}

/* ── Load R2 helper (provides r2_url, r2_delete, r2_handle_upload, r2_upload, r2_mime) ── */
$_r2_path = __DIR__ . '/r2_helper.php';
if (file_exists($_r2_path) && !function_exists('r2_upload')) {
    require_once $_r2_path;
}
unset($_r2_path);
...
if (!function_exists('r2_url')) {
    ...
    if (empty($key)) return '';
    return 'https://pub-5721570094064d529f1527519424c77b.r2.dev/dms_uploads/' . ltrim($key, '/');
}
}
...
/* ── despatchFilter: return SQL WHERE fragment to restrict despatch rows by agent ── */
function despatchFilter(string $alias = ''): string {
    if (function_exists('isAdmin') && isAdmin()) return '';
    if (function_exists('canViewAll') && canViewAll('despatch')) return '';
    $uid = (int)($_SESSION['user_id'] ?? 0);
    if (!$uid) return '';
    $col = $alias ? $alias.'.created_by' : 'created_by';
    return " AND $col=$uid";
}
...
function canViewAll(string $module): bool {
    ...
    $col = match($module) {
        ...
        'transporter_payments'  => 'view_all_transporter_payments',
        'transporter_bills'     => 'view_all_transporter_bills',
        'agent_commissions'     => 'view_all_agent_commissions',
    ...
    $res = $db->query("SELECT `$col` FROM app_users WHERE id=$uid LIMIT 1");
    if (!$res) return false;
    $row = $res->fetch_assoc();
    return (bool)($row[$col] ?? false);
}

/* ── agentFilter: restricts agent_commissions queries to current user ── */
function agentFilter(string $alias = 'ac'): string {
    if (function_exists('isAdmin') && isAdmin()) return '';
    if (function_exists('canViewAll') && canViewAll('despatch')) return '';
    $uid = (int)($_SESSION['user_id'] ?? 0);
    if (!$uid) return '';
    $col = $alias ? $alias.'.agent_id' : 'agent_id';
    return " AND $col=$uid";
}
...
function sanitize($data) {
    ...
    $db = getDB();
    return $db->real_escape_string(trim(htmlspecialchars((string)$data, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')));
}
